...
function Interview_chart({ accumulatedEmotions, onEmotionFeedback }) {
  ...
  // 감정 값을 0-1 범위로 변환
  const convertToOneScale = (data) => {
    const total = Object.values(data).reduce((sum, value) => sum + value, 0);
    const oneScaleData = {};
    Object.keys(data).forEach((key) => {
      oneScaleData[key] = (data[key] / total).toFixed(2); // 0-1 범위로 변환하고 소수점 2자리까지 표시
    });
    return oneScaleData;
  };

  // 변환된 데이터를 사용하여 N각형 차트 데이터 생성
  const generateChartData = (data) => {
    const oneScaleData = convertToOneScale(data);
    const labels = Object.keys(oneScaleData);
    const values = Object.values(oneScaleData);

    return {
      labels,
      datasets: [
        {
          label: 'Emotion Scores (0-1)',
          data: values,
    ...
    <Box className="interview-results-container" sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', p: 2 }}>
      <Card className="interview-results-card" sx={{ width: '100%', maxWidth: { xs: '100%', md: 800 }, boxShadow: 3, borderRadius: 2 }}>
        <CardContent>
            ...
            <Radar
              ...
              options={{
                ...
                scales: {
                  r: {
                    min: 0,
                    max: 1, // 0에서 1까지 범위로 표시
                    ticks: {
                      stepSize: 0.2, // 0.2 간격으로 표시
                    },
                  },
                },
              }}
          ...
          {/* 감정 점수 목록 */}
          <Box sx={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-around', mb: 2 }}>
            {Object.entries(convertToOneScale(accumulatedEmotions)).map(([emotion, score]) => (
              <Box key={emotion} sx={{ textAlign: 'center', p: 1, minWidth: 80 }}>
                <Typography variant="body1" color="text.secondary" sx={{ fontWeight: 'bold' }}>
                  {emotion}
                </Typography>
